function PerceptualGroupingSequential
...
% Stimuli are divided over the two displays.
...
fileName = [p.root,filesep,'Subject Data',filesep,num2str(p.subNum), '_PerceptualGroupingSequential.mat'];
if p.subNum ~= 0
...
%----------------------------------------------------
p.portCodeNumbers = [11:14,19:22,100:106];
p.portCodeLabels = {'SS2 Same First','SS2 Same Second','SS2 Different First',...
    'SS2 Different Second','SS4 Same First','SS4 Same Second',...
    'SS4 Different First','SS4 Different Second',...
...
stim.colors1 = cell(prefs.numTrials,prefs.numBlocks); % color originally presented at the probed location
stim.colors2 = cell(prefs.numTrials,prefs.numBlocks); % color originally presented at the probed location
...
if p.eyeTrack
    ...
    edfFile=[num2str(p.subNum),'_PGS.edf']; %File name MUST BE LESS THAN 8 characters!!!!!
...
for b = 1:prefs.numBlocks
    ...
    for t = 1:prefs.numTrials
        ...
        stim.probeItem = changeIndex(1);
        nItems = setSize/2;
        ITI = .01*randi([60 150],1); % random interval of 600 - 1500 ms
        ...
        % calculate locations
        [xPos1,yPos1,xPos2,yPos2] = getStimLocs(prefs,win,setSize,condition);
        
        stim.x1{t,b} = xPos1; stim.y1{t,b} = yPos1;
        stim.x2{t,b} = xPos2; stim.y2{t,b} = yPos2;
        
        colorIndex1 = randperm(size(win.colors_9,1));
        stim.colors1{t,b} = colorIndex1(1:setSize/2);
        colorIndex2 = colorIndex1(setSize/2+1:end);
        stim.colors2{t,b} = colorIndex2(1:setSize/2);
...
%         stim.changeInd(t,b) = changeIndex;       
    ...
    if b == prefs.numBlocks;
        ...
        Screen('TextFont',win.onScreen,'Arial');
        DrawFormattedText(win.onScreen, 'Finished! Please see the experimenter.', 'center', 'center', [255 255 255]);
        Screen('Flip', win.onScreen);
...
InstructImage = imread([pwd,'/Instructions_Sequential'],'png','BackgroundColor',[win.gray/255,win.gray/255,win.gray/255]);
...
InstructText = ['1. Keep your eyes on the center fixation circle. \n'...
    ...
    '4. Remember the color(s) of any additional squares that appear. \n'...
    'Additional squares might occupy the same locations as squares already in memory. \n'...
...
%-------------------------------------------------------------------------
function [xPos1,yPos1,xPos2,yPos2] = getStimLocs(prefs,win,setSize,condition)

clear xPos1 yPos1 xPos2 yPos2
...
% scatter(0,0)
% 
% leftborder = [xGrid(:)-50,yGrid(:)];
% scatter(leftborder(:,1),leftborder(:,2),'k')
% rightborder = [xGrid(:)+50,yGrid(:)];
% scatter(rightborder(:,1),rightborder(:,2),'k')
% topborder = [xGrid(:),yGrid(:)+50];
% scatter(topborder(:,1),topborder(:,2),'k')
% bottomborder = [xGrid(:),yGrid(:)-50];
% scatter(bottomborder(:,1),bottomborder(:,2),'k')
% 
% for i = 1:50
% scatter(leftborder(:,1),leftborder(:,2)+i,'k')
% scatter(leftborder(:,1),leftborder(:,2)-i,'k')
% 
% scatter(rightborder(:,1),rightborder(:,2)+i,'k')
% scatter(rightborder(:,1),rightborder(:,2)-i,'k')
% 
% scatter(topborder(:,1)+i,topborder(:,2),'k')
% scatter(topborder(:,1)-i,topborder(:,2),'k')
% 
% scatter(bottomborder(:,1)+i,bottomborder(:,2),'k')
% scatter(bottomborder(:,1)-i,bottomborder(:,2),'k')
% end
...
firstDispCenters = datasample(nCenters,setSize/2,'Replace',false); % randomly pick stimuli locs
...
%     secondDispQuads = datasample(nQuads,setSize/2,'Replace',false); % randomly pick quadrant(s) for second display wo replacement
%     if nCenters == 1
%         secondDispQuads = datasample(nQuads(~ismember(nQuads,firstDispQuads)),setSize/2,'Replace',false); % pick remaining two quads if only 1x1 grid
%     end
    secondDispCenters = datasample(nCenters(~ismember(nCenters,firstDispCenters)),setSize/2,'Replace',false); % randomly pick stimuli locs within quadrant
...
for l = 1:length(xPos1)
    if condition == 1
        rAng = randperm(360,1);
        xPos1(l) = round(xPos1(l) + (jitter*cosd(rAng)));
        yPos1(l) = round(yPos1(l) + (jitter*sind(rAng)));

        xPos2(l) = round(xPos2(l) + (jitter*cosd(rAng)));
        yPos2(l) = round(yPos2(l) + (jitter*sind(rAng)));
    else
        rAng = randperm(360,1);
        xPos1(l) = round(xPos1(l) + (jitter*cosd(rAng)));
        yPos1(l) = round(yPos1(l) + (jitter*sind(rAng)));
...
% Design conditions
prefs.numBlocks = 20;
prefs.nTrialsPerCondition = 4;
prefs.conditions = [1 2]; % 1 = Same, 2 = Different
