...
class BaseExperiment:
    ...
    # overwriting template class's methods - want to extend
    def _check_file_conflict(self, screen=0):
        existing_file_options = {'choice': [
            'Extend',
            'Overwrite',
            'New File'
        ]}
        existingFileChoice = psychopy.gui.DlgFromDict(
            existing_file_options,
            title='How to Handle Old File Existing?',
            screen=screen
        )
        
        self.overwrite_ok = existing_file_options['choice']=='Overwrite'
        self.extend_ok = existing_file_options['choice']=='Extend'
    ...
    def get_experiment_info_from_dialog(self, additional_fields_dict=None, screen=0):
        ...
        self.experiment_info = {'Subject Number': '999',
                                'Age': '99',
                                'Experimenter Initials': 'HJ',
                                'Unique Subject Identifier': '999999'
                                }
    ...
    def init_experiment_fileBase(self, fileBase=None):
        if fileBase is None:
            fileBase = (self.experiment_name + '_' +
                        self.experiment_info['Subject Number'].zfill(3))
        
                    
        if len(glob.glob(fileBase)) > 0: # check for sub dir
            self._check_file_conflict()
            if not (self.overwrite_ok or self.extend_ok):
                # If the file exists make a new filename
                i = 1
                new_fileBase = fileBase + '(' + str(i) + ')'
                while len(glob.glob(new_fileBase)) > 0:
                    i += 1
                    new_fileBase = fileBase + '(' + str(i) + ')'
    ...
    def save_experiment_info(self, filename=None):
        ...
        if filename is None:
            filename = self.fileBase + '_info' + ext
        elif filename[-5:] != ext:
    ...
    def open_csv_data_file(self, data_filename=None):
        ...
        if data_filename is None:
            data_filename = self.fileBase + ext
        elif data_filename[-4:] != ext:
            data_filename = data_filename + ext

        self.experiment_data_filename = data_filename
        # Write the header if you're not extending from a previous session
    ...
    def save_experiment_pickle(self, additional_fields_dict=None):
        ...
        pickle_filename = self.fileBase + '.pickle'
