...
class Experiment:
    def __init__(
        ...
        info_variable_names=['chan_labels', 'chan_x', 'chan_y', 'chan_z', 'sampling_rate', 'times'],
...
class Interpreter:
    ...
    def plot_acc_by_subject(self, subtitle='', significance_testing=False, stim_time=[0, 250],
                 savefig=False, title=None, ylim=[.18, 1], chance_text_y=.19,chance=None,subplot_dims=None,sub_list=None,figSize=(15,10)):
        '''
        Plots classification accuracy per subject
        ** significance testing is currently broken
        '''
        if subplot_dims is None:
            subplot_dims=[1,self.acc.shape[0]]

        if sub_list is None:
            sub_list=range(self.acc.shape[0])
        ...
        for isub,sub in enumerate(sub_list):
            ...
            acc = np.mean(self.acc[sub], 1)
            se = sista.sem(acc, 0)
            upper_bound, lower_bound = acc + se, acc - se
            acc_shuff = np.mean(self.acc_shuff[sub], 1)
            se_shuff = sista.sem(acc_shuff, 0)
            upper_bound_shuff, lower_bound_shuff = acc_shuff + \
                se_shuff, acc_shuff - se_shuff
            if chance is None:
            ...
            # plotting
            # ax=plt.subplot(111)
            ax = plt.subplot(*subplot_dims,isub+1)
            ...
            ax.plot(self.t, acc, color='tab:red', linewidth=2)
            ...
            ax.text(0.82, chance_text_y, 'Shuffle', transform=ax.transAxes, fontsize=12,
                    verticalalignment='top', color='grey')
            ax.text(0.25, .98, 'Stim', transform=ax.transAxes, fontsize=14,
                    verticalalignment='top', color='white')
            if title is not None:
                plt.title(title + ' - s' + str(sub), fontsize=16)

            self.savefig('acc'+subtitle, save=savefig)

            delay_period_acc = np.mean(acc[self.t > stim_time[1]])
            delay_period_sd = np.std(acc[self.t > stim_time[1]])
            ax.text(0.9,0.9,f'delay acc: {round(delay_period_acc,2)}',transform=ax.transAxes, fontsize=16,ha='right')
            # plt.show()
    ...
    def plot_acc(self, subtitle='', significance_testing=False, stim_time=[0, 250],
        ...
        ax.fill_between(self.t, upper_bound, lower_bound,
                        alpha=.5, color='k')
        ax.plot(self.t, acc_mean, color='k', linewidth=2)
...
class ERP:
    ...
    def plot_ss(self, xdata_all, ydata_all, subtitle='', ax=None,
                ...
                savefig=False, file_format=['.png', '.pdf'], colors=None):
        ...
        for iss, ss in enumerate(condition_subset):
            ...
            ax.plot(self.info['times'], x,
                    label=f'{ss}', linewidth=2.5, alpha=0.75, color=colors[iss])
            # SE
            ax.fill_between(self.info['times'], x-se, x+se, alpha=.25, color=colors[iss])
